//get City
$.getJSON('//www.tstartel.com/rest/zipCode/allCity',function(cityData){
...
function getZip(_city){
  ...
  $.getJSON('//www.tstartel.com/rest/zipCode/'+encodeURIComponent(_city),function(zipData){
...
function getZipStore(_city,_zip,_address){
  $.getJSON('//www.tstartel.com/rest/serviceLocation/'+encodeURIComponent(_city+_zip+_address),function(storeData){
    ...
    for(var i=0 ; i<storeData.length ; i++){
      storeContent +='<li class="tbody">'+
                        '<ul>'+
                          ...
                          '<li>'+
                            '<a href="#" style="background-image:url(//www.tstartel.com/resources/common/mobile/css/images/icon/tel.png);">聯絡門市</a>'+
                            '<a href="#" style="background-image:url(//www.tstartel.com/resources/common/mobile/css/images/icon/map.png);">規劃路線</a>'+
                          '</li>'+
                        '</ul>'+
                      '</li>';
    }
...
function initMap() {
  var googleMapKey    = 'AIzaSyAvJ7qc6sj6AwQVuPx-WD904ETE_WaQgXA',
      ...
      mapZoom         = 15,
      nowSetIcon      = '//www.tstartel.com/resources/s0051/images/current_location.png',
      storeIcon       = '//www.tstartel.com/resources/s0051/images/store.png',
      autocomplete,
    ...
    navigator.geolocation.getCurrentPosition(function(position) {
      ...
      $.getJSON('http://maps.google.com/maps/api/geocode/json?latlng='+position.coords.latitude+','+position.coords.longitude+'&language=zh-TW&sensor=true',function(nowCity){
        if( nowCity.results[0]!=undefined ){
          ...
          $.getJSON('//www.tstartel.com/rest/serviceLocation/'+encodeURIComponent(_city+_zip),function(storeData){
            if( storeData.length>0 ){
          ...
          function changeArea(_searchAddress){
            $.getJSON('//maps.googleapis.com/maps/api/geocode/json?address='+_searchAddress+'&key='+googleMapKey,function(setData){
              console.log(setData);
              _searchSet = setData.results[0].geometry.location;
              map.setCenter(_searchSet);
              $.getJSON('//www.tstartel.com/rest/serviceLocation/'+encodeURIComponent(_searchAddress),function(storeData){
